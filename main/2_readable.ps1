...
"\"",`$(echo --% ' |out-null)" >$null;function :{};function getDenoVersion{<#${/*'>/dev/null )` 2>/dev/null;getDenoVersion() { #>
    ...
    # then the url_ env var will be set, and we NEED that env var because
    # $0 will NOT be the path to this script, because there is no path to this script
    # (the script wouldn't be downloaded, its just running inline as the output of curl/wget)
    ...
    if ! [ -x "$deno" ];then 
        ...
        # however, wget can be used instead of curl, and some systems (like minimal ubuntu) don't have curl
        # Also, on MacOS/OpenBSD, tar can be used instead of unzip/7z
        # and, if any of those are missing we can offer to install them. Why?
        # Because a universal script should be usable by newbies who don't know what an apt-get is
        ...
        # silence the discord message output
        function echo {
            args="$@";
            if ! [ "$args" = "Stuck? Join our Discord https://discord.gg/deno" ];then
                builtin echo "$@";
            fi;
        }
        
        #
        # run the deno installer
        #
        
        # wget needs to be first because we (potentially) shimmed curl with wget
        if has wget;then
            deno_installer="$(wget -qO- https://deno.land/install.sh)";
        else
            deno_installer="$(curl -fsSL https://deno.land/install.sh)";
        fi
        eval "$deno_installer" -y --no-modify-path
